...
resource "aws_lambda_function" "lambda_trigger" {
  function_name = "lambda_trigger-scheduled-lambda"
  ...
  source_code_hash = filebase64sha256("lambda_function.zip") # This ensures that if the local file changes, Terraform will detect it and update the Lambda function code accordingly.
}
