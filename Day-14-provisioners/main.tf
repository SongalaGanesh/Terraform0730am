...
resource "aws_key_pair" "example" {
  key_name   = "task"
 public_key = file("C:/Users/hp/Downloads/Telegram Desktop/lin_key.pub")
}
...
resource "aws_instance" "server" {
  ...
  connection {
    ...
    private_key = file("c:/Users/hp/Downloads/Telegram_Desktop/lin_key.pem")             # Path to private key
    host        = self.public_ip
    timeout     = "2m"
  }
...
resource "null_resource" "run_script" {
  provisioner "remote-exec" {
    connection {
      ...
      user        = "ubuntu"
      private_key = file("c:/Users/hp/Downloads/Telegram_Desktop/lin_key.pem")
    }
